...
export const buildChooseTechnologyHandler =
	...
	async (args: {
		...
		const candidates = Object.values(technologies).filter(
			(tech) =>
				typeof tech?.title === 'string' && typeof tech?.identifier === 'string',
		);
		...
		if (normalizedIdentifier) {
			chosen = candidates.find(
				(tech) => tech.identifier?.toLowerCase() === normalizedIdentifier,
			);
		}
		...
		if (!chosen && normalizedName) {
			chosen = candidates.find(
				(tech) => tech.title?.toLowerCase() === normalizedName,
			);
		}
