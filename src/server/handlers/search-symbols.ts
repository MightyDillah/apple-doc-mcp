...
} from '../services/framework-loader.js';
...
const scoreEntry = (entry: {tokens: string[]; ref: ReferenceData}, terms: string[]): number => {
	let score = 0;
...
const performHierarchicalSearch = async (
...
const performRegexSearch = async (
	context: ServerContext,
	query: string,
	frameworkName: string,
	maxResults: number,
	...
	return async (args: {maxResults?: number; platform?: string; query: string; symbolType?: string}): Promise<ToolResponse> => {
		...
		if (usedFallback && fallbackResults.length > 0) {
			...
			if (hierarchicalCount > 0) {
				...
				if (regexCount > 0) fallbackTitle += `, ${regexCount} fuzzy`;
				if (directCount > 0) fallbackTitle += `, ${directCount} direct`;
				fallbackTitle += ')';
